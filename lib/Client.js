...
var modPath = path.join(__dirname, 'bin', process.platform+ '-'+ process.arch+ '-'+ process.versions.modules, 'sqlclient');
var addon = require(modPath+'.node');
...
Client.prototype.query = function(str, values, config, cb) {
  var req;
  var ret;
  if (typeof str !== 'string') throw new Error('Missing query string');
  ...
  if (typeof cb === 'function') {
    ...
    req = {
      emitter: new EventEmitter(this._handle),
      stream: undefined,
      str: str,
      needColumns: needColumns,
      needMetadata: needMetadata,
...
Client.prototype._onerror = function(err) {
    ...
    var req = this._req;
	if (!req && this._queue.length > 0) {
		req = this._req = this._queue.shift();
	}
    if (req) {
      if (req.cb !== undefined) {
...
Client.prototype._onresultinfo = function(cols, metadata) {
  ...
  if (req.cb !== undefined) {
    ...
    if (req.needMetadata) {
	  req.emitter.emit('metadata', createMetadata(metadata));
    }
  }
  if (req.needColumns)
